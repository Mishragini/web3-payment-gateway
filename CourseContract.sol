...
contract CourseContract is Ownable{
    ...
    event PaymentRecieved(address user, string email, uint256 amount);
    ...
    function getPaymentsByUser(address userAdress) public view returns(Payment[] memory) {
        uint256 count = 0;
        for(uint i=0; i < payments.length; i++){
            if(payments[i].user == userAdress){
                count++;
            }
        }
